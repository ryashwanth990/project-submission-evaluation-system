              ...
              {evaluations.map((evalRecord) => (
                <div key={evalRecord.id} className="p-6 hover:bg-gray-50/50 transition-colors">
                  <div className="flex flex-col sm:flex-row justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-xs text-gray-500">{format(new Date(evalRecord.createdAt), "MMM d, yyyy")}</span>
                      </div>
                  ...
                  <div className="grid grid-cols-4 gap-4 mt-4 pt-4 border-t text-xs">
                    <div><span className="text-gray-500">Innovation:</span> <span className="font-medium">{evalRecord.innovationScore ?? '-'}</span></div>
                    <div><span className="text-gray-500">Technical:</span> <span className="font-medium">{evalRecord.technicalScore ?? '-'}</span></div>
                    <div><span className="text-gray-500">Presentation:</span> <span className="font-medium">{evalRecord.presentationScore ?? '-'}</span></div>
                    <div><span className="text-gray-500">Docs:</span> <span className="font-medium">{evalRecord.documentationScore ?? '-'}</span></div>
                  </div>
                </div>
              ))}
            </div>
